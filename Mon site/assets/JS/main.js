...
// Mode Matrix ---------------------------------------------------------------------
function matrixify() {
    // On récupère le texte chargé
    const text = document.getElementById("fileDisplayArea").innerText;

    // On efface toute la page (sauf <body>)
    document.body.innerHTML = '<div id="matrixZone"></div>';

    // On applique le style Matrix
    ...
    function typeNext() {
        if (i < text.length) {
            ...
            i++;
            // On attend un court instant avant d’écrire la lettre suivante
            setTimeout(typeNext, Math.random() * 50 + 15); // vitesse aléatoire pour effet Matrix
        }
    }

    typeNext(); // Démarre l'effet
}
